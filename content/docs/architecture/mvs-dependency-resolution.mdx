...
| **Registry Volatility** | `latest` resolution or registry updates can silently alter build output over time. | SemVer requirements select `max(requested_minimums)`; versionless Git requirements are pinned on first tidy. Subsequent runs reuse the lock. |
...
1. **Version Bounds or Git HEAD**: A SemVer requirement (for example, `github.com/acme/weather v1.2.0`) supplies a minimum version. A requirement without a version resolves Git HEAD and stores the commit in the lock.
...
The maximum-version rule applies to SemVer requirements. A versionless requirement resolves the repository's current HEAD on its first `mod tidy`; later tidy runs reuse the locked commit, and `mod update` refreshes it. A full Git commit hash pins an exact revision without a tag. If the same module is required by two different commits, or by both a commit and a SemVer tag, resolution reports a conflict because those requirements have no common version ordering.
...
- **`module`**: Declares the required canonical module identity (URL/path).
...
- **`require`**: Declares direct and indirect module dependencies. The version may be omitted for automatic Git HEAD resolution, or set to a Semantic Version tag or full Git commit hash.
...
- **`replace`**: Overrides a module dependency with a local file path (`../weather-fork`) for development.
