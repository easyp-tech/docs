...
The maximum-version rule applies to SemVer requirements. A full Git commit hash pins an exact revision without a tag. If the same module is required by two different commits, or by both a commit and a SemVer tag, resolution reports a conflict because those requirements have no common version ordering.
...
- **`require`**: Declares direct and indirect module dependencies with explicit Semantic Version tags or full Git commit hashes.
