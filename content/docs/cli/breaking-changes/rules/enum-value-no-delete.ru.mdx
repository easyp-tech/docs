...
enum OrderStatus {
  ...
  ORDER_STATUS_SHIPPED = 3;
  ORDER_STATUS_DELIVERED = 4;
  // ORDER_STATUS_CANCELLED = 5; // Удалено значение enum — BREAKING!
}
...
enum Priority {
  ...
  PRIORITY_MEDIUM = 2;
  PRIORITY_HIGH = 3;
  // PRIORITY_URGENT = 4; // Удалено значение enum — BREAKING!
}
```
...
enum Priority {
  ...
  PRIORITY_MEDIUM = 2;
  PRIORITY_HIGH = 3;
  PRIORITY_URGENT = 4 [deprecated = true];
}
```
