...
message SearchRequest {
  ...
  oneof filter {
    int32 category = 2; // Было string -> стало int32 (BREAKING)
    string user_id = 3; // Было int32 -> стало string (BREAKING)
    string is_premium = 4; // Было bool -> стало string (BREAKING)
    string date_range = 5; // Было DateRange -> стало string (BREAKING)
  }
}

message NotificationSettings {
  oneof delivery {
    EmailConfig email = 1; // Было string -> стало EmailConfig (BREAKING)
    string phone_number = 2; // Было int64 -> стало string (BREAKING)
    string webhook = 3; // Было WebhookConfig -> стало string (BREAKING)
  }
}
```
