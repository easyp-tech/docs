...
message Product {
  string name = 1;
  string price = 2; // Changed from int32 to string (изменён тип)
  bool available = 3;
}
```
...
message Product {
  string name = 1;
  int32 price = 2 [deprecated = true]; // Старое поле помечено deprecated
  bool available = 3;
  string price_formatted = 4; // Новое поле со строковым представлением
}
```
...
package myapi.v2; // Новая версия package

message Product {
  string name = 1;
  string price = 2; // В v2 уже string
  bool available = 3;
}
```
