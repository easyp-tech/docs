...
message Product {
  string name = 1;
  string price = 2; // Changed from int32 to string
  bool available = 3;
}
```
...
message Product {
  string name = 1;
  int32 price = 2 [deprecated = true];
  bool available = 3;
  string price_formatted = 4; // New field for string price
}
```
...
message Product {
  string name = 1;
  string price = 2; // Now string in v2
  bool available = 3;
}
```
