...
message LoginRequest {
  ...
  oneof credentials {
    string password = 2;
    string api_key = 3;
    // string oauth_token = 4; // Удалено поле oneof — BREAKING!
    // string certificate = 5; // Удалено поле oneof — BREAKING!
  }
}
...
message SearchQuery {
  oneof filter {
    string text = 1;
    int32 user_id = 2;
    // string category = 3; // Удалено поле oneof — BREAKING!
    // DateRange date_range = 4; // Удалено поле oneof — BREAKING!
  }
}
```
...
message LoginRequest {
  ...
  oneof credentials {
    string password = 2;
    string api_key = 3;
    string oauth_token = 4 [deprecated = true]; // Старое поле
    string certificate = 5 [deprecated = true];
    
    OAuthCredentials oauth_v2 = 6; // Новый структурированный вариант
