...
service UserService {
  rpc GetUser(GetUserRequestV2) returns (GetUserResponse); // Changed request type
  rpc UpdateUser(UserUpdateRequest) returns (UpdateUserResponse); // Changed request type
}

message GetUserRequestV2 { // Different message type
  string id = 1;
  bool include_profile = 2;
}

message UserUpdateRequest { // Different message type
  string id = 1;
...
service UserService {
  ...
  rpc GetUserV2(GetUserRequestV2) returns (GetUserResponse); // New RPC method
  rpc UpdateUser(UpdateUserRequest) returns (UpdateUserResponse) {
    option deprecated = true;
  }
  rpc UpdateUserV2(UserUpdateRequest) returns (UpdateUserResponse); // New RPC method
}
