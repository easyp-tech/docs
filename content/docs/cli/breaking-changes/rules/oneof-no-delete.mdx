...
message PaymentMethod {
  oneof method {
    option deprecated = true;
    CreditCard credit_card = 1 [deprecated = true];
    BankAccount bank_account = 2 [deprecated = true];
    PayPal paypal = 3 [deprecated = true];
  }
}
```
...
message LoginRequest {
  ...
  oneof credentials {
    option deprecated = true; // Old oneOf
    string password = 2 [deprecated = true];
    string api_key = 3 [deprecated = true];
    string oauth_token = 4 [deprecated = true];
  }
  
  oneof auth_method { // New oneOf with better design
    PasswordAuth password_auth = 5;
    ApiKeyAuth api_auth = 6;
    OAuthAuth oauth_auth = 7;
  }
}
```
