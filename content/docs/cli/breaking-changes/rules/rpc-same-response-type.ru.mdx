...
service UserService {
  rpc GetUser(GetUserRequest) returns (GetUserResponseV2); // Изменён тип ответа
  rpc UpdateUser(UpdateUserRequest) returns (UserUpdateResponse); // Изменён тип ответа
}

message GetUserResponseV2 { // Другой тип сообщения
  string id = 1;
  UserProfile profile = 2;
  repeated string permissions = 3;
}

message UserUpdateResponse { // Другой тип сообщения
...
service UserService {
  ...
  rpc GetUserV2(GetUserRequest) returns (GetUserResponseV2); // Новый метод
  rpc UpdateUser(UpdateUserRequest) returns (UpdateUserResponse) {
    option deprecated = true;
  }
  rpc UpdateUserV2(UpdateUserRequest) returns (UserUpdateResponse); // Новый метод
}
