...
message User {
  string name = 1;
  optional string email = 2; // Было required (implicit), стало optional
  int32 age = 3;
}

message CreateUserRequest {
  string name = 1;
  string email = 2; // Было optional, стало требуемым (implicit required)
  string phone = 3;
}
```
...
message User {
  string name = 1;
  string email = 2 [deprecated = true]; // Сохраняем старую кардинальность
  int32 age = 3;
  optional string email_optional = 4; // Новое optional поле
}
...
message CreateUserRequest {
  ...
  string phone = 3;
  string email_required = 4; // Новое требуемое поле
}
```
...
message User {
  string name = 1;
  optional string email = 2; // Чистая модель в v2
  int32 age = 3;
}

message CreateUserRequest {
  string name = 1;
  string email = 2; // Требуемое в v2
  string phone = 3;
}
```
