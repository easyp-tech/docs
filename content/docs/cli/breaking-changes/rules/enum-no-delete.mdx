...
message Order {
  string id = 1;
  OrderStatus status = 2;
  UserRole created_by_role = 3 [deprecated = true];
}
```
...
enum UserRole {
  option deprecated = true; // Old enum
  USER_ROLE_UNSPECIFIED = 0;
  USER_ROLE_ADMIN = 1;
  USER_ROLE_USER = 2;
}

enum UserPermission { // New enum with better design
...
message Order {
  string id = 1;
  OrderStatus status = 2;
  UserRole created_by_role = 3 [deprecated = true]; // Old field
  UserPermission created_by_permission = 4; // New field
}
```
