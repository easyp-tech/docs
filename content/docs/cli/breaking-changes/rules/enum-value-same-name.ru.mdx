...
enum OrderStatus {
  ORDER_STATUS_UNSPECIFIED = 0;
  ORDER_STATUS_PENDING = 1;
  ORDER_STATUS_APPROVED = 2; // Было ORDER_STATUS_CONFIRMED
  ORDER_STATUS_SHIPPED = 3;
  ORDER_STATUS_COMPLETED = 4; // Было ORDER_STATUS_DELIVERED
}
```
...
enum OrderStatus {
  ...
  ORDER_STATUS_APPROVED = 5; // Новое значение вместо переименования
...
enum OrderStatus {
  ORDER_STATUS_UNSPECIFIED = 0;
  ORDER_STATUS_PENDING = 1;
  ORDER_STATUS_APPROVED = 2; // «Чистые» имена в v2
  ORDER_STATUS_SHIPPED = 3;
