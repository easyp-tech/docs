...
message SearchRequest {
  ...
  oneof filter {
    int32 category = 2; // Changed from string to int32 - BREAKING!
    string user_id = 3; // Changed from int32 to string - BREAKING!
    string is_premium = 4; // Changed from bool to string - BREAKING!
    string date_range = 5; // Changed from DateRange to string - BREAKING!
  }
}

message NotificationSettings {
  oneof delivery {
    EmailConfig email = 1; // Changed from string to EmailConfig - BREAKING!
    string phone_number = 2; // Changed from int64 to string - BREAKING!
    string webhook = 3; // Changed from WebhookConfig to string - BREAKING!
  }
}
```
...
message SearchRequest {
  ...
  oneof filter {
    string category = 2 [deprecated = true]; // Keep original type
    int32 user_id = 3 [deprecated = true]; // Keep original type
    bool is_premium = 4 [deprecated = true]; // Keep original type
    DateRange date_range = 5 [deprecated = true]; // Keep original type
    
    int32 category_id = 6; // New field with desired type
    string user_identifier = 7; // New field with desired type
    string premium_status = 8; // New field with desired type
    string date_filter = 9; // New field with desired type
  }
}
```
...
message SearchRequest {
  ...
  oneof filter {
    option deprecated = true; // Deprecate old oneOf
  ...
  oneof filter_v2 { // New oneOf with correct types
    int32 category_id = 6;
