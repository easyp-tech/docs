...
message LoginRequest {
  ...
  oneof credentials {
    string password = 2;
    string api_key = 3;
    // string oauth_token = 4; // Deleted oneOf field - BREAKING!
    // string certificate = 5; // Deleted oneOf field - BREAKING!
  }
}
...
message SearchQuery {
  oneof filter {
    string text = 1;
    int32 user_id = 2;
    // string category = 3; // Deleted oneOf field - BREAKING!
    // DateRange date_range = 4; // Deleted oneOf field - BREAKING!
  }
}
```
...
message LoginRequest {
  ...
  oneof credentials {
    ...
    string certificate = 5 [deprecated = true];
  }
}
...
message SearchQuery {
  oneof filter {
    ...
    DateRange date_range = 4 [deprecated = true];
  }
}
```
...
message LoginRequest {
  ...
  oneof credentials {
    string password = 2;
    string api_key = 3;
    string oauth_token = 4 [deprecated = true]; // Keep old field
    string certificate = 5 [deprecated = true]; // Keep old field
    
    OAuthCredentials oauth_v2 = 6; // New structured approach
    CertificateCredentials cert_v2 = 7; // New structured approach
  }
}
```
