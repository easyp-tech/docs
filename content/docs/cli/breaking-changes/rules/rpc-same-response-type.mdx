...
service UserService {
  rpc GetUser(GetUserRequest) returns (GetUserResponseV2); // Changed response type
  rpc UpdateUser(UpdateUserRequest) returns (UserUpdateResponse); // Changed response type
}

message GetUserResponseV2 { // Different message type
  string id = 1;
  UserProfile profile = 2;
  repeated string permissions = 3;
}

message UserUpdateResponse { // Different message type
...
service UserService {
  ...
  rpc GetUserV2(GetUserRequest) returns (GetUserResponseV2); // New RPC method
  rpc UpdateUser(UpdateUserRequest) returns (UpdateUserResponse) {
    option deprecated = true;
  }
  rpc UpdateUserV2(UpdateUserRequest) returns (UserUpdateResponse); // New RPC method
}
...
package myapi.v2; // New package version

service UserService {
  rpc GetUser(GetUserRequest) returns (GetUserResponse); // Clean interface in v2
  rpc UpdateUser(UpdateUserRequest) returns (UpdateUserResponse);
}

message GetUserResponse {
  string id = 1; // Improved field names
  UserProfile profile = 2;
  repeated string permissions = 3;
}
```
