...
service UserService {
  ...
  rpc UpdateUser(UserUpdateRequest) returns (UpdateUserResponse); // Изменён тип запроса
}

message GetUserRequestV2 { // Новый тип запроса
  string id = 1;
  bool include_profile = 2;
}

message UserUpdateRequest { // Новый тип запроса
  string id = 1;
...
service UserService {
  ...
  rpc GetUserV2(GetUserRequestV2) returns (GetUserResponse); // Новый метод
  rpc UpdateUser(UpdateUserRequest) returns (UpdateUserResponse) {
    option deprecated = true;
  }
  rpc UpdateUserV2(UserUpdateRequest) returns (UpdateUserResponse); // Новый метод
}
