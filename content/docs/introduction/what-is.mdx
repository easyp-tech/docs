...
<Callout type="info">
Just want to try it out? Skip to the [Quickstart](/docs/introduction/quickstart).
</Callout>
...
## Using Buf-configured dependencies

An EasyP v1 project declares Git dependencies in `protobuf.mod`. When a Git dependency uses Buf v1 or v2 configuration, EasyP reads its declared proto roots for imports. Older EasyP dependency configuration is supported in the same way. The consuming project still uses separate `protobuf.mod`, `easyp.gen.yaml`, and `easyp.yaml` files.

See [Buf-configured Git dependencies](/docs/cli/package-manager/easyp-vs-buf) for supported formats and limits.
...
Ready to get started? Check out our [Installation Guide](/docs/introduction/install) and [Quickstart Tutorial](/docs/introduction/quickstart).
