...
plugin easyp-tech publishes. In v1.0.2 that is **80 plugins with 1738 versions**
in ten groups:
...
Counted with `find registry -name plugin.yaml | wc -l` and by summing the
`versions:` lists. `ls registry/<group>` in your checkout is the current list.
...
**`push --force` after register breaks the checksum.** The object changes under
the recorded hash, and the next download fails with
`plugin archive checksum mismatch`. Register again after re-pushing.
