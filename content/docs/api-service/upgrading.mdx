---
title: "Upgrading"
description: "Release-by-release upgrade notes for the API Service: what breaks, what it looks like when it does, and what to change."
---
...
From v1.0.0 the surfaces below are fixed. Changing any of them is a major
version, so they are listed here rather than left to be inferred from what
happens not to have changed yet.

**Frozen**

- The proto package `easyp.generator.v1` and the service name `GeneratorAPI`.
- Field names and field numbers in every message. A removed field keeps its
  number reserved; a number is never reused.
- The `reason` values in `google.rpc.ErrorInfo`, under domain `easyp.tech`.
- Configuration keys, their environment spellings, and what a value means.
- The public API of the `sdk` and `api` Go modules.

**Still open without a major version**

- New fields with new numbers, new RPCs, new messages.
- New configuration keys, and new `reason` values.
- New SDK options and new methods.
- Raising a licence ceiling. Lowering one is not: a limit introduced where none
  existed takes throughput away from deployments that already have it, which is
  why the ceilings that exist were drawn before 1.0 rather than after.

A rename is handled without breaking anyone: the old configuration key keeps
working for one release while warning, naming both spellings and the line to
change, and only then stops.

## v1.0.0 → v1.0.2

### The image checks its own health

From v1.0.1 the image carries a `HEALTHCHECK` that runs `easyp-svc health`,
which probes `/live` on the health port. The probe has to know that port. It
reads the same configuration the service does, so a port set through the
environment is found without further work. A port set in a YAML file is not:
the file is named by `--cfg` on the `start` command line, and the probe does not
see that command line.

From v1.0.2 every command that accepts `--cfg` also reads the path from
`EASYP_CONFIG`. Set it on the container, and both the service and the probe
use the same file:
...
What it looks like without it: the container serves normally while `docker ps`
reports `unhealthy`, and the probe's log says
`dial tcp 127.0.0.1:23412: connect: connection refused`. Docker's `restart`
policies ignore health, so nothing restarts, but anything that waits on
`condition: service_healthy` never proceeds. Kubernetes ignores the image's
`HEALTHCHECK` altogether; the chart's probes are unaffected.
...
outcome, but it can surface as a plugin that "suddenly broke": find out why the
object changed, then restore the original archive or delete the version and
push and register it again (registering over an existing version is skipped).
...
dependency was removed rather than worked around.
...
The `.spec/` directory was removed. Runbooks moved out of the repository
and are now published at [Runbooks](/docs/api-service/runbooks), which is where
the `runbook_url` on every alert points. Links into `.spec/` from anywhere else
are dead.
