...
description: "What a backup of the API Service has to contain, how long to keep it, and the order a restore has to happen in."
---
...
3. **Check `goose_db_version` against the binary you are restoring onto.** A
   ...
   this is not caught. goose objects only to migrations missing *below* the
   database's highest version; one it has never heard of, above that mark,
   leaves it with nothing to apply and no complaint. Pinned by
   `TestRollbackOntoAnOlderBinary`.
...
5. **Verify audit continuity before declaring done.** Partition maintenance runs
   ...
   [Runbooks](/docs/api-service/runbooks).
