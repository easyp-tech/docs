...
```
 developer / CI                      EasyP API Service                         storage
┌──────────────┐  GenerateCode   ┌──────────────────────────────┐        ┌──────────────────┐
│  easyp CLI   │ ───────────────▶│ gRPC listener (TLS / mTLS)   │        │ PostgreSQL       │
│  or Go SDK   │  plugin name +  │  rate limit · concurrency    │◀──────▶│  plugin registry │
│              │  CodeGenerator- │  auth · licence              │        │  audit log (Ent.)│
│              │  Request        │            │                 │        └──────────────────┘
│              │                 │  worker pool ─ resolve plugin│        ┌──────────────────┐
│              │                 │            │  (cache miss) ──┼───────▶│ S3-compatible    │
│              │                 │  child process: plugin       │        │  plugin archives │
│              │◀─────────────── │  stdin → stdout              │        └──────────────────┘
└──────────────┘ CodeGenerator-  └──────────────────────────────┘
                 Response
```
